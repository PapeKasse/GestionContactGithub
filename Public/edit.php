...
if ($_SESSION==null) {
    ...
    if (isset($_GET['id'])) {
        ...
        $requser = $user->getConnexion()->prepare("SELECT * FROM contacts WHERE id = ? and users_id='".$_SESSION['id']."'");
        $requser->execute([$id]);
        $info = $requser->fetch();
        if($info){
          $_SESSION['id_url']=$id;
          $_SESSION['id_base']=$info['id'];
        if (!is_numeric(($_SESSION['id_url'])) ||  !is_numeric(($_SESSION['id_base']))) {
           return false;
        }
        }else {
          return header('location:profil.php?id='.$_SESSION['id']);
        }
        
    }
    if ($_SESSION['id_url']==$_SESSION['id_base']) {
        if (!empty($_POST['nom']) and !empty($_POST['prenom']) and !empty($_POST['tel'])) {
            $nom = htmlspecialchars(trim($_POST['nom']));
            $prenom = htmlspecialchars(trim($_POST['prenom']));
            $tel = intval(htmlspecialchars(trim($_POST['tel'])));
            $sql= $user->db->prepare("UPDATE contacts SET nom= ?,prenom= ?, tel= ? WHERE id=$id ");
            $sql->execute([$nom,$prenom,$tel]);
...
<div class="container">
    <div class="col-md-7 col-lg-8">
        <form method="Post" class="needs-validation" onsubmit='return validation();'>
              ...
              <input type="text" class="form-control" id="firstName" placeholder="" value="<?php echo $info['nom'] ;?>" required name="nom" required pattern="[A-Za-z]+">
              
            
              <label for="lastName" class="form-label">Prenom</label>
              <input type="text" class="form-control" id="lastName" placeholder="" value="<?php echo $info['prenom'] ;?>" required name="prenom" required pattern="[A-Za-z]+">
              
              <label for="Telephone" class="form-label">tel <span class="text-muted"></span></label>
              <input type="number" class="form-control" id="" placeholder="" name="tel" value="<?php echo $info['tel'] ;?>" required name="tel" required pattern="[0-9]+"  minlength="9"></br>
              
           <button class="btn btn-primary" type="submit" name="envoyer">Editer</button>  
        </form>
